class Node{
  ...
  class Cell{
    ...
    mouseAttraction(){
      ...
      let centerPoint = this.getCenterPoint();
      //if(p5.Vector.dist(mousePoint,centerPoint)<this.radius){
      //  return;
      //}
      for(let i = 0; i<this.nodes.length; i++){
    ...
    update(){
      ...
      this.gravity();
      this.walls();
    //   cool fibers arise as the cells self-avoid
      ...
      for(let i = 0; i<this.nodes.length; i++){
        tempNodes[i].position.add(this.nodes[i].velocity);
        tempNodes[i].velocity.add(this.nodes[i].acceleration);
        tempNodes[i].velocity.limit(2);
        //tempNodes[i].velocity.mult(0.8);
        tempNodes[i].acceleration = createVector(0,0);
      }
