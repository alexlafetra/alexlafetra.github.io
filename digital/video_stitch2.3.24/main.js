...
const imageArray = ['images/IMG_4677.MOV','images/IMG_4678.MOV','images/IMG_1749.JPG','images/IMG_1858.jpeg','images/IMG_1749.JPG','images/IMG_1755.jpeg'];
...
function reset(){
  ...
  for(let i = 0; i<NUMBER_OF_HORIZONTAL_THREADS; i++){
    // horThreads.push(new ClothThread(-(width-2*border)/2+i*(width-2*border)/NUMBER_OF_HORIZONTAL_THREADS+(width-2*border)/(NUMBER_OF_HORIZONTAL_THREADS*2),-(height-2*border)/2,color(0),i,DOWN));
    horThreads.push(new ClothThread(-(width-2*border)/2+i*(width-2*border)/NUMBER_OF_HORIZONTAL_THREADS+(width-2*border)/(NUMBER_OF_HORIZONTAL_THREADS*2),-(height-2*border)/2,color(floor(i/vStripeWidth)%imageArray.length),i,DOWN));
  }
  for(let i = 0; i<NUMBER_OF_VERTICAL_THREADS; i++){
    vertThreads.push(new ClothThread(-(width-2*border)/2,-(height-2*border)/2+i*(height-2*border)/NUMBER_OF_VERTICAL_THREADS+(height-2*border)/(NUMBER_OF_VERTICAL_THREADS*2),color(floor(i/hStripeWidth)%imageArray.length),i,RIGHT));
    // vertThreads.push(new ClothThread(-(width-2*border)/2,-(height-2*border)/2+i*(height-2*border)/NUMBER_OF_VERTICAL_THREADS+(height-2*border)/(NUMBER_OF_VERTICAL_THREADS*2),color(floor(i/hStripeWidth)%imageArray.length),i,RIGHT));
  }
...
function draw() {
  ...
  stitchShader.setUniform('uThreshold',imageThresholdSlider.value());
  quad(-1,-1,1,-1,1,1,-1,1);
  frameBuffer.end();
