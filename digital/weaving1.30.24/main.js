...
class AnniLine{
  ...
  renderStitchBuffer(){
    if(this.done)
      return;
    let gap = this.threadGap*noise(2+this.age/10);
    stitchPatternBuffer_anniLine.begin();
    noiseSeed(this.seed);
    noFill();
    ...
    strokeWeight(this.thickness*noise(this.age/10));
    line(this.points[this.points.length-1].x-gap,this.points[this.points.length-1].y-gap,this.points[this.points.length-2].x-gap,this.points[this.points.length-2].y-gap);
    stitchPatternBuffer_anniLine.end();
  }
  growAcrossScreen(){
    while(this.grow()){
      this.render();
...
let stitchPatternBuffer_anniLine;
...
//threshold controlling whether or not a pixel is interpreted as an
//up or a down stitch
let imageThreshold = 0.5;
...
function reset(){
  ...
  stitchPatternBuffer_anniLine = createFramebuffer();
  ...
  // let pal1 = new WeaveColor(randomPallette());
  // let pal2 = new WeaveColor(randomPallette());
  // let pal1 = new WeaveColor([color(255)]);
  // let pal2 = new WeaveColor([color(0)]);
  let pal1 = new WeaveColor([color(255,0,0)]);
  let pal2 = new WeaveColor([color(0,0,255)]);
  ...
  for(let i = 0; i<NUMBER_OF_HORIZONTAL_THREADS;i++){
    for(let j = 0; j<NUMBER_OF_VERTICAL_THREADS;j++){
      fill(((count%2)||(random(1)<stitchErrorChance))?'white':'black');
  ...
  image(stitchPatternBuffer,-stitchPatternBuffer_masked.width/2,-stitchPatternBuffer_masked.height/2,stitchPatternBuffer_masked.width,stitchPatternBuffer_masked.height);
...
function loadStitchImage(bmp){
  stitchPatternBuffer.begin();
  image(bmp,-stitchPatternBuffer.width/2,-stitchPatternBuffer.height/2,stitchPatternBuffer.width,stitchPatternBuffer.height);
  stitchPatternBuffer.end();
}
...
function updateStitchImageFromWebcam(){
  ...
  imageMaskShader.setUniform('uThreshold',imageThreshold);
...
function updateStitchPattern(){
  if(mouse){
    let x = floor(mouseX/width*NUMBER_OF_HORIZONTAL_THREADS);
    let y = floor(mouseY/height*NUMBER_OF_VERTICAL_THREADS);
    stitchPatternBuffer.begin();
    fill(shift?color(255):color(0));
    noStroke();
    rect(x*width/NUMBER_OF_HORIZONTAL_THREADS-width/2,y*height/NUMBER_OF_VERTICAL_THREADS-height/2,threadDensity,threadDensity);
    stitchPatternBuffer.end();
  }
}

function draw() {
  imageThreshold = imageThresholdSlider.value();
  ...
  frameBuffer.begin();
  shader(stitchShader);
  stitchShader.setUniform('uStitchPattern',stitchPatternBuffer_masked);
  stitchShader.setUniform('uThreshold',imageThreshold);
  ...
  shader(stitchShader);
  stitchShader.setUniform('uStitchPattern',stitchPatternBuffer_anniLine);
  stitchShader.setUniform('uThreshold',imageThreshold);
  ...
  image(frameBuffer2,-width/2,-height/2,width,height);
  // image(stitchPatternBuffer_masked,-width/2,-height/2,width,height);
}
