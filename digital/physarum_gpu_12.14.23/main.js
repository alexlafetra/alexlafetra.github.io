...
function updateVel(){
    ...
    updateVelShader.setUniform('uTrails',trailLayer);
    updateVelShader.setUniform('uSenseAngle',sensorAngle);//GLSL uses degrees! don't convert these to radians
    updateVelShader.setUniform('uSenseDist',sensorDistance/10);
    updateVelShader.setUniform('uFrameCount',frameCount);
...
function draw(){
    ...
    for(let i = 0; i<min(NUMBER_OF_PARTICLES,positionTexture.pixels.length/4); i++){
        ...
        // stroke(i/NUMBER_OF_PARTICLES*255,i/NUMBER_OF_PARTICLES*255,255);
        point(x*width-width/2,y*height-height/2);
    }
    trailLayer.end();

    // image(simplexLayer,-width/2,-height/2,width,height);

    // blurring the trails to simulate diffusion
    // renderLayer.begin();
    // clear();
    // shader(blurShader);
    // blurShader.setUniform('uTexture',trailLayer);
    // blurShader.setUniform('uResolution',[width,height]);
    // quad(-1,-1,1,-1,1,1,-1,1);//upside down bc the textures get flipped
    // renderLayer.end();

    image(trailLayer,-width/2,-height/2,width,height);
    // image(renderLayer,-width/2,-height/2,width,height);
    image(velTexture,-width/2,-height/2,velTexture.width/2,velTexture.height/2);
