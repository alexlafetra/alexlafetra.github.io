...
class FlowField{
    constructor(points,mask,colors,flowFShader){
        ...
        this.updateVelPhysarumShader = createShader(physarumVert,physarumFrag);
    ...
    updatePhysarum(){
        this.velTextureBuffer.begin();
        shader(this.updateVelPhysarumShader);
        this.updateVelPhysarumShader.setUniform('uSensorAngle',25.0);
        this.updateVelPhysarumShader.setUniform('uSensorDistance',0.1);

        this.updateVelPhysarumShader.setUniform('uParticleVel',this.velTexture);
        this.updateVelPhysarumShader.setUniform('uParticlePos',this.uPositionTexture);
        this.updateVelPhysarumShader.setUniform('uTrailTexture',this.trailLayer);
        quad(-1,-1,1,-1,1,1,-1,1);//upside down bc the textures get flipped
        this.velTextureBuffer.end();
        [this.velTexture,this.velTextureBuffer] = [this.velTextureBuffer,this.velTexture];
    }
...
void main(){
    ...
    vec2 particleLocation = texture2D(uParticlePos,vParticleCoord).xy;
    vec2 particleVel = texture2D(uParticleVel,vParticleCoord).xy;

    //calculate which way the particle is facing/moving
    float currentHeading = dot(vec2(0.0,1.0),normalize(particleVel));

    //calculate sensor locations
    float sensorDistance = 0.01;
    float sensorAngle = 30.0;
    vec2 sensor1 = particleLocation+vec2(uSensorDistance*cos(-uSensorAngle+currentHeading),uSensorDistance*sin(-uSensorAngle+currentHeading));
    vec2 sensor2 = particleLocation+vec2(uSensorDistance*cos(currentHeading),uSensorDistance*sin(currentHeading));
    vec2 sensor3 = particleLocation+vec2(uSensorDistance*cos(uSensorAngle+currentHeading),uSensorDistance*sin(uSensorAngle+currentHeading));

    //get sensor values
    float val1 = texture2D(uTrailTexture,sensor1).x;
    float val2 = texture2D(uTrailTexture,sensor2).x;
    float val3 = texture2D(uTrailTexture,sensor3).x;
    ...
    turnForce = turnForce - particleLocation;
    gl_FragColor = vec4(normalize(turnForce+particleVel).xy,1.0,1.0);
    // gl_FragColor = vec4(1.0);
}
`;