...
// const imageArray = ['images/IMG_4677.MOV','images/IMG_4678.MOV'];
...
function reset(){
  ...
  for(let i = 0; i<NUMBER_OF_HORIZONTAL_THREADS; i++){
    horThreads.push(new ClothThread(-width/2+i*width/NUMBER_OF_HORIZONTAL_THREADS+width/(NUMBER_OF_HORIZONTAL_THREADS*2),-height/2,color(floor(i/vStripeWidth)%imageArray.length),i,DOWN));
  }
  for(let i = 0; i<NUMBER_OF_VERTICAL_THREADS; i++){
    vertThreads.push(new ClothThread(-width/2,-height/2+i*height/NUMBER_OF_VERTICAL_THREADS+height/(NUMBER_OF_VERTICAL_THREADS*2),color(floor(i/hStripeWidth)%imageArray.length),i,RIGHT));
  }
...
function draw() {
  ...
  // imageShader.setUniform('uSourceImages',sourceImages);//This array isn't working for some reason
  ...
  // adding in the anniline wanderer
  frameBuffer2.begin();
  ...
  stitchShader.setUniform('uHorzStitches',finalPass);
  quad(1,-1,-1,-1,-1,1,1,1);
  frameBuffer2.end();
