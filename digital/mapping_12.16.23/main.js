...
function preload(){
    ...
    gradientShader = loadShader('flowMap.vert','flowMap.frag');
...
function getLargestFive(){
    let biggest5 = [];
    let j = 0;
    for(let tract of bayTracts){
        let whitePopulation2000 = tract.data2000.get('White');
        let whitePopulation2020 = tract.data2020.get('White');
        let blackPopulation2000 = tract.data2000.get('Black');
        let blackPopulation2020 = tract.data2020.get('Black');
        let totalPopulation2000 = tract.data2000.get('Total');
        let totalPopulation2020 = tract.data2020.get('Total');

        let stat = whitePopulation2000/totalPopulation2000 - whitePopulation2020/totalPopulation2020;
        for(let i = 0; i<biggest5.length; i++){
            if(stat>biggest5[i].value || !biggest5[i].value){
                let statObject = {value:stat,index:j};
                biggest5.insert(stat,i);
                break;
            }
        }
        j++;
...
function fillPointsAndWeights(){
    pointsAndWeights = [];
    let biggest5 = getLargestFive();
    for(let i = 0; i<biggest5.length; i++){
        pointsAndWeights[i] = biggest5[i];
    }
}

function mousePressed(){
    // fillPointsAndWeightsWithRandom();
    let coord = createVector(mouseX-width/2,mouseY-height/2);
    clickClosestCentroid(coord);
    console.log("Tract: "+bayTracts[indexOfClickedTract].properties.TRACTCE);
    console.log(bayTracts[indexOfClickedTract]);
    printDemographics(bayTracts[indexOfClickedTract]);
}
...
function renderGradient(geometryOffset,scale){
    gradientBuffer.begin();
    strokeWeight(1);
    for(let x = -width/2; x<width/2; x++){
        for(let y = -height/2; y<height/2; y++){
            let color = calculateAvgWeightedDistance((x+geometryOffset.x)*scale.x,(y+geometryOffset.y)*scale.y)*1;
            stroke(color);
            point(x,y);
        }
    }
...
function mostWhiteChange(a,b){
    let changeA = a.whitePopulation2020/a.totalPopulation2020 - a.whitePopulation2000/a.totalPopulation2000;
    let changeB = b.whitePopulation2020/b.totalPopulation2020 - b.whitePopulation2000/b.totalPopulation2000;
...
function setup(){
    ...
    mapTexture2 = createFramebuffer(width,height);
    mapTexture3 = createFramebuffer(width,height);
    mapTexture4 = createFramebuffer(width,height);
...
function draw(){
    ...
    gradientShader.setUniform('uPoint1',[pointsAndWeights[0],pointsAndWeights[1],pointsAndWeights[2]]);
    gradientShader.setUniform('uPoint2',[pointsAndWeights[3],pointsAndWeights[4],pointsAndWeights[5]]);
    gradientShader.setUniform('uPoint3',[pointsAndWeights[6],pointsAndWeights[7],pointsAndWeights[8]]);
    gradientShader.setUniform('uPoint4',[pointsAndWeights[9],pointsAndWeights[10],pointsAndWeights[11]]);
    gradientShader.setUniform('uPoint5',[pointsAndWeights[12],pointsAndWeights[13],pointsAndWeights[14]]);
    ...
    // image(gradientBuffer,-width/2,-height/2,width,height);
